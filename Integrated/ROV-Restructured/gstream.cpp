...
// JPEG compressing pipeline (JPEG is frame by frame , requiring larger bandwidth but less delay )
    pipeline_description="udpsrc port="+QString::number(PORT).toStdString()+" ! application/x-rtp,encoding-name=JPEG,payload=26 ! rtpjpegdepay ! jpegdec ! xvimagesink name=sink  force-aspect-ratio=false";
